...
export class InvoiceService {
  ...
  async findAll(params: InvoiceListDto) {
    ...
    try {
      const qb = this.invoiceRepo.createQueryBuilder('inv')
        .orderBy(
          sortBy === 'total' ? 'inv.total' : 'inv.createdAt',
  ...
  async create(dto: CreateInvoiceDto, userId: string) {
    ...
    const lines: Partial<InvoiceLine>[] = dto.lineItems.map((li) => {
      ...
      taxTotal += tax
      return {
        tenantId: '1', companyId: '1', description: li.description,
        qty: li.quantity, rate: li.unitPrice, amount, createdBy: userId,
      }
    })
    ...
    try {
      ...
      await this.audit.log(userId, 'CREATE_INVOICE', 'invoice', saved.invoiceId, `Invoice ${invoiceNo} created`, 'success')
      return this.format(saved)
    } catch {
      return this.mockInvoice(invoiceNo, dto, subtotal, taxTotal, total, userId)
    }
  }

  async send(id: string, userId: string) {
    try {
      await this.invoiceRepo.update({ invoiceId: id }, { status: 'Issued', updatedBy: userId })
      await this.audit.log(userId, 'SEND_INVOICE', 'invoice', id, 'Invoice sent to client', 'success')
  ...
  async getCreditNotes(_params: InvoiceListDto) {
    return { data: [], total: 0 }
  }

  async getDebitNotes(_params: InvoiceListDto) {
    return { data: [], total: 0 }
  }
  ...
  private format(inv: Invoice) {
    return {
      ...
      lineItems: (inv.lines ?? []).map((li) => ({
        ...
        quantity: Number(li.qty), unitPrice: Number(li.rate),
  ...
  private mockInvoice(invoiceNo: string, dto: CreateInvoiceDto, subtotal: number, taxTotal: number, total: number, _userId: string) {
    return {
      id: 'mock-' + Date.now(), invoiceNumber: invoiceNo, type: 'Tax', status: 'Draft',
      lineItems: dto.lineItems.map((li, i) => ({
        id: `li-${i}`, description: li.description, quantity: li.quantity, unitPrice: li.unitPrice,
        amount: li.quantity * li.unitPrice,
      })),
      subtotal, taxTotal, total, balance: total,
