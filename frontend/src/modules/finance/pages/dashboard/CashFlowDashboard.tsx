    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 3 }}>Liquidity, inflows and outflows · all amounts in INR</div>
        </div>
        {cashHealth?.lastPaymentAt && (
          <span style={{ fontSize: 11, color: '#64748b', background: '#F5F0EB', padding: '5px 12px', borderRadius: 20 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a2a4a 0%, #2d4a7a 100%)', borderRadius: 14, padding: '24px 28px', marginBottom: 20, color: '#fff' }}>
        <div style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', color: 'rgba(255,255,255,0.5)', marginBottom: 8 }}>Cash Health</div>
        {isLoading
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', padding: 20, marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>Last 6 weeks of received payments vs projected burn</div>
          </div>
        ...
        {isLoading
          ...
          : weeklyForecast.some((w) => w.historical > 0 || w.projected > 0)
            ? (
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={weeklyForecast} barSize={28}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => formatINR(v, { compact: true })} width={60} />
                  <Tooltip formatter={(v: number, n) => [formatINR(v), n === 'historical' ? 'Received' : 'Projected burn']} contentStyle={{ borderRadius: 8, border: '1px solid #E8E0D8', fontSize: 11 }} />
            ...
            : <div style={{ textAlign: 'center', padding: '32px 0', color: '#94a3b8', fontSize: 13 }}>No recent cash activity</div>
        }
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, marginBottom: 20 }}>
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px 10px' }}>
            ...
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>Payments received and vendor payouts disbursed</div>
          </div>
          ...
          {movements.length === 0 && <div style={{ padding: '28px', textAlign: 'center', color: '#94a3b8', fontSize: 12 }}>No cash movements yet</div>}
          {movements.map((m, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 110px 120px 130px', padding: '14px 20px', alignItems: 'center', borderTop: i === 0 ? 'none' : '1px solid #F5F0EB' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#1a2a4a' }}>{m.label}</div>
              <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 4, width: 'fit-content', background: m.kind === 'Inflow' ? '#D1FAE5' : '#FEE2E2', color: m.kind === 'Inflow' ? '#065F46' : '#991B1B' }}>{m.kind}</span>
              <div style={{ fontSize: 12, color: '#64748b' }}>{dayjs(m.date).format('MMM DD, YYYY')}</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: m.amount >= 0 ? '#059669' : '#DC2626' }}>{m.amount >= 0 ? '+' : '−'}{formatINR(Math.abs(m.amount))}</div>
            </div>
          ))}
        </div>

        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#1a2a4a', marginBottom: 14 }}>Strategic Insights</div>
          {insights.length === 0 && <div style={{ color: '#94a3b8', fontSize: 12 }}>No insights available</div>}
          {insights.map((ins, i) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          {[
            ...
            <div key={row.label} style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: '#1a2a4a' }}>{formatINR(row.amount, { compact: true })}</span>
              </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', padding: 20 }}>
          ...
          {(branches ?? []).length === 0 && <div style={{ color: '#94a3b8', fontSize: 12 }}>No branch data</div>}
          {(branches ?? []).map((b: any, i: number) => (
            <div key={b.branchId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderTop: i === 0 ? 'none' : '1px solid #F5F0EB' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 12, color: '#334155' }}>{b.branchName}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: '#1a2a4a' }}>{formatINR(b.revenue, { compact: true })}</span>
                <span style={{ fontSize: 10, color: '#8B1A1A', background: '#FBEAEA', padding: '2px 6px', borderRadius: 4, fontWeight: 600 }}>{b.sharePct}%</span>
              </div>
            </div>
          ))}
        </div>
