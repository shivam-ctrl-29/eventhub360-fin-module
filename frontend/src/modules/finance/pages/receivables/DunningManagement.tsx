      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 14, marginBottom: 20 }}>
        {[{ label: 'Level 1 — Soft Remind', count: l1, color: '#C4A24D' }, { label: 'Level 2 — Follow-up', count: l2, color: '#E2946B' }, { label: 'Level 3 — Legal', count: l3, color: '#8B1A1A' }].map((l) => (
          <div key={l.label} style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', padding: '16px 18px', borderLeft: `4px solid ${l.color}` }}>
            ...
            <div style={{ fontSize: 12, color: '#64748b' }}>client{l.count !== 1 ? 's' : ''}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}>
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#1a2a4a' }}>Dunning Queue</div>
            <button onClick={handleSendAll} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, color: '#fff', border: 'none', borderRadius: 6, padding: '5px 12px', background: '#8B1A1A', cursor: 'pointer' }}>
              <SendOutlined /> Send All Reminders
            </button>
          </div>

          {isLoading && <div style={{ padding: 20 }}><Skeleton active paragraph={{ rows: 4 }} /></div>}
          {!isLoading && queue.length === 0 && <div style={{ padding: '32px', textAlign: 'center', fontSize: 14, color: '#94a3b8' }}>No overdue accounts in queue</div>}
              ...
              <div key={d.id} onClick={() => setSelectedId(selectedId === d.id ? null : d.id)} style={{ padding: '16px 20px', borderTop: i === 0 ? 'none' : '1px solid #F5F0EB', cursor: 'pointer', background: selectedId === d.id ? 'rgba(139,26,26,0.03)' : '#fff' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 4, background: lvlStyle.bg, color: lvlStyle.color, marginTop: 4, display: 'inline-block' }}>
                      {d.dunningLevel}
                    </span>
                  </div>
                </div>
                {selectedId === d.id && (
                  <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                    <button onClick={(e) => { e.stopPropagation(); handleSend(d.customerId, d.dunningLevel) }} style={{ padding: '6px 14px', borderRadius: 6, border: 'none', background: '#8B1A1A', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Send Now</button>
                    <button onClick={(e) => { e.stopPropagation(); viewHistory(d) }} style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #E8E0D8', background: '#fff', color: '#334155', fontSize: 12, cursor: 'pointer' }}>View History</button>
                  </div>
                )}
              </div>
            )
          ...
          {[{ level: 'L1', label: 'Soft Reminder', days: '1-7 days', icon: '📧' }, { level: 'L2', label: 'Follow-up Call', days: '8-30 days', icon: '📞' }, { level: 'L3', label: 'Demand Letter', days: '31-60 days', icon: '📜' }, { level: 'L4', label: 'Legal Action', days: '60+ days', icon: '⚖️' }].map((s, i) => (
            <div key={s.level} style={{ display: 'flex', gap: 12, marginBottom: 16, position: 'relative' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{s.days} overdue</div>
              </div>
            </div>
          ))}
        </div>
