  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 3 }}>Detailed receivables aging analysis by bucket and client</div>
      </div>
        ...
        {summaryLoading
          ...
          : bucketCards.map((b, idx) => (
            <div key={b.bucket} style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', padding: '16px 18px', borderTop: `3px solid ${BUCKET_COLORS[idx] ?? '#94a3b8'}` }}>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: '#94a3b8', marginBottom: 6 }}>{b.bucket}</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: '#1a2a4a' }}>{formatINR(b.amount, { compact: true })}</div>
              <div style={{ fontSize: 11, color: '#64748b', marginTop: 4 }}>{b.count} clients</div>
            </div>
          ))
        }
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#1a2a4a', marginBottom: 16 }}>Aging Distribution</div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#1a2a4a', marginBottom: 16 }}>Risk Summary</div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', overflow: 'hidden' }}>
        ...
        {!entriesLoading && entries.map((c, i) => (
          <div key={c.customerId} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 110px 80px', padding: '14px 20px', alignItems: 'center', borderTop: i === 0 ? 'none' : '1px solid #F5F0EB' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#1a2a4a' }}>{c.customerName}</div>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#1a2a4a' }}>{formatINR(c.total)}</div>
        ...
        {!entriesLoading && entries.length === 0 && (
          <div style={{ padding: '32px', textAlign: 'center', fontSize: 13, color: '#94a3b8' }}>No aging entries found</div>
        )}
      </div>
    </div>
  )
