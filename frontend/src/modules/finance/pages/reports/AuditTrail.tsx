  ...
  if (!can('audit.view')) {
    return (
      <div style={{ padding: '48px 24px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 14, color: '#94a3b8', marginTop: 6 }}>You do not have permission to view the Audit Trail.</div>
      </div>
    )
  }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: '#94a3b8', marginTop: 3 }}>Complete immutable log of all financial actions and system events</div>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid #E8E0D8', borderRadius: 8, padding: '7px 12px', width: 240 }}>
            ...
            <input value={localSearch} onChange={(e) => setLocalSearch(e.target.value)} placeholder="Search logs..." style={{ border: 'none', outline: 'none', fontSize: 13, background: 'transparent', width: '100%', color: '#334155' }} />
          </div>
          <button onClick={() => {
          ...
          }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, border: '1px solid #E8E0D8', background: '#fff', fontSize: 13, color: '#334155', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {['All', 'info', 'success', 'warning', 'error'].map((s) => (
          <button key={s} onClick={() => setSeverityFilter(s === 'All' ? '' : s)} style={{ padding: '5px 14px', borderRadius: 20, border: '1px solid #E8E0D8', fontSize: 12, fontWeight: 600, cursor: 'pointer', background: (s === 'All' ? '' : s) === severityFilter ? '#8B1A1A' : '#fff', color: (s === 'All' ? '' : s) === severityFilter ? '#fff' : '#64748b', textTransform: 'capitalize' }}>
              ...
              <div key={log.id} style={{ position: 'relative', marginBottom: 16 }}>
                ...
                <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #E8E0D8', padding: '14px 18px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr 120px 100px 90px', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#1a2a4a', marginBottom: 2 }}>{log.action}</div>
                      <div style={{ fontSize: 12, color: '#64748b' }}>{log.description}</div>
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#8B1A1A', fontFamily: 'monospace' }}>{log.entityId}</div>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#334155' }}>{log.user}</div>
                    </div>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: style.bg, color: style.color, textAlign: 'center', textTransform: 'capitalize' }}>
                      {log.severity}
                    </span>
                  </div>
                </div>
