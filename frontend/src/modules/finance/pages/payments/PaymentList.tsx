...
const INPUT: React.CSSProperties = { width: '100%', padding: '8px 12px', border: '1px solid #E8E0D8', borderRadius: 8, fontSize: 13, color: '#334155', outline: 'none', boxSizing: 'border-box' }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: '#94a3b8', marginTop: 3 }}>Track advances, instalments and milestone payments</div>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid #E8E0D8', borderRadius: 8, padding: '7px 12px', width: 220 }}>
            ...
            <input value={localSearch} onChange={(e) => handleSearch(e.target.value)} placeholder="Search payments..." style={{ border: 'none', outline: 'none', fontSize: 13, background: 'transparent', width: '100%', color: '#334155' }} />
          </div>
          {can('payment.create') && (
            <button onClick={openRecordModal} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, border: 'none', background: '#8B1A1A', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, padding: '12px 0' }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#64748b', display: 'block', marginBottom: 5 }}>Invoice ID *</label>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#64748b', display: 'block', marginBottom: 5 }}>UTR / Cheque No.</label>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8E0D8', overflow: 'hidden' }}>
        ...
        {!isLoading && payments.map((p, i) => {
          ...
          return (
            <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '140px 1.5fr 140px 130px 130px 110px 110px', padding: '14px 20px', alignItems: 'center', borderTop: i === 0 ? 'none' : '1px solid #F5F0EB' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#8B1A1A' }}>{(p as any).paymentNumber ?? `PAY-${p.id}`}</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#1a2a4a' }}>{(p as any).customerName ?? `Invoice #${p.invoiceId}`}</div>
              <div style={{ fontSize: 12, color: '#64748b' }}>{(p as any).invoiceNumber ?? (p as any).invoiceId ?? '—'}</div>
              <div><span style={{ fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 4, background: modeStyle.bg, color: modeStyle.color, textTransform: 'uppercase' }}>{((p as any).paymentMode ?? p.mode ?? '—').replace('_', ' ')}</span></div>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#1a2a4a' }}>{formatINR(p.amount)}</div>
              <div style={{ fontSize: 12, color: '#64748b' }}>{dayjs((p as any).paymentDate ?? (p as any).paidAt ?? p.createdAt).format('MMM DD, YYYY')}</div>
              <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: statusStyle.bg, color: statusStyle.color, textTransform: 'capitalize' }}>{(p as any).status ?? 'completed'}</span>
            </div>
          )
        })}
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid #F5F0EB', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: '#94a3b8' }}>{isLoading ? 'Loading...' : `Showing ${payments.length} of ${total} payments`}</span>
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button disabled={payments.length < pageSize} onClick={() => setCurrentPage(currentPage + 1)} style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid #E8E0D8', background: '#fff', cursor: payments.length < pageSize ? 'not-allowed' : 'pointer', fontSize: 14, color: '#334155', opacity: payments.length < pageSize ? 0.4 : 1 }}>›</button>
          </div>
        </div>
      </div>
    </div>
