...
export default function KPICard({ title, value, subValue, badge, badgeColor = '#059669', topBorderColor }: KPICardProps) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: 6 }}>
        {title}
      </div>
      <div style={{ fontSize: 22, fontWeight: 700, color: '#1a2a4a', lineHeight: 1.2 }}>{value}</div>
      {subValue && <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>{subValue}</div>}
      {badge && (
        <span style={{
